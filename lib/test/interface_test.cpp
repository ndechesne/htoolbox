...
int main(void) {
  ...
  Node meta("test_r/test1/testfile");
  ...
  sys_rc = system("rm -f test_db/.data/0f/2e/a9/73d77135dc3d06c8e68da6dc59-0/data.gz");
  sys_rc = system("rm -f test_db/.data/b9/0f/8f/a56ea1d39881d4a199c7a81d35-0/data.gz");
  sys_rc = system("rm -f test_db/.data/fe/f5/18/38cd3cfe8ed96355742eb71fbd-0/data.gz");
  hbackup = new HBackup();
  if (hbackup->open("etc/hbackup.conf")) {
    return 1;
  }
  ...
  sys_rc = system("mkdir -p test_db/.data/00/00/00/00000000000000000000000000-0");
  sys_rc = system("touch test_db/.data/00/00/00/00000000000000000000000000-0/data");
  sys_rc = system("echo 1 > test_db/.data/00/00/00/00000000000000000000000000-0/meta");
  sys_rc = system("mkdir -p test_db/.data/88/88/88/88888888888888888888888888-0");
  sys_rc = system("touch test_db/.data/88/88/88/88888888888888888888888888-0/data");
  sys_rc = system("echo 1000 > test_db/.data/88/88/88/88888888888888888888888888-0/meta");
  sys_rc = system("mkdir -p test_db/.data/ff/ff/ff/ffffffffffffffffffffffffff-0");
  sys_rc = system("touch test_db/.data/ff/ff/ff/ffffffffffffffffffffffffff-0/data");
  sys_rc = system("echo 1000000 > test_db/.data/ff/ff/ff/ffffffffffffffffffffffffff-0/meta");
  sys_rc = system("mkdir -p test_db/.data/55/55/55/55555555555555555555555555-0");
  sys_rc = system("touch test_db/.data/55/55/55/55555555555555555555555555-0/data.gz");
  hbackup = new HBackup();
  if (hbackup->open("etc/hbackup.conf")) {
    return 1;
  }
  ...
  sys_rc = system("mkdir -p test_db/.data/33/33/33/33333333333333333333333333-0");
  sys_rc = system("touch test_db/.data/33/33/33/33333333333333333333333333-0/data");
  sys_rc = system("echo 3 > test_db/.data/33/33/33/33333333333333333333333333-0/meta");
  sys_rc = system("mkdir -p test_db/.data/77/77/77/77777777777777777777777777-0");
  sys_rc = system("touch test_db/.data/77/77/77/77777777777777777777777777-0/data");
  sys_rc = system("echo 7 > test_db/.data/77/77/77/77777777777777777777777777-0/meta");
  sys_rc = system("mkdir -p test_db/.data/dd/dd/dd/dddddddddddddddddddddddddd-0");
  sys_rc = system("touch test_db/.data/dd/dd/dd/dddddddddddddddddddddddddd-0/data");
  sys_rc = system("echo d > test_db/.data/dd/dd/dd/dddddddddddddddddddddddddd-0/data");
  hbackup = new HBackup();
  if (hbackup->open("etc/hbackup.conf")) {
    return 1;
  }
  ...
  sys_rc = system("gzip test_db/.data/52/52/f2/42d27b8c2c9fdbdcbb33545d07-0/data");
  sys_rc = system("rm test_db/.data/52/52/f2/42d27b8c2c9fdbdcbb33545d07-0/meta");
  // Wrong gzip format, data should be compressed => conflict
  sys_rc = system("gunzip test_db/.data/81/6d/f6/f64deba63b029ca19d880ee10a-0/data.gz");
  sys_rc = system("gzip test_db/.data/81/6d/f6/f64deba63b029ca19d880ee10a-0/data");
  ...
  hbackup->close();
  delete hbackup;
  sys_rc = system("ls test_db/.data/6d/7f/ce/9fee471194aa8b5b6e47267f03-0");
  showClientConfigs();

  cout << endl << "Test: second backup should not recover again" << endl;
  // Wrong gzip format and corrupted => what?
  sys_rc = system("gzip test2/testfile -c > test_db/.data/fb/00/cd/74a5f35e89a7fbdd3c1d05375a-0/data");
  sys_rc = system("echo 185 > test_db/.data/fb/00/cd/74a5f35e89a7fbdd3c1d05375a-0/meta");
  hbackup = new HBackup();
  if (hbackup->open("etc/hbackup.conf")) {
    return 1;
  }
